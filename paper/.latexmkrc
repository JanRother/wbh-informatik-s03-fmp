...
sub generate_date_metadata {
    if (defined $ENV{'DATE_MODE'} && $ENV{'DATE_MODE'} ne '') {
        my $script = 'scripts/generate-date.sh';
        my $status = system($script);

        if ($status != 0) {
            die "ERROR: Date metadata generation failed with status $status\n.";
        }
    }
}
